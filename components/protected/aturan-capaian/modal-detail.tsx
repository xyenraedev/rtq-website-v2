...
export function ModalDetail({
  ...
  const [confirmLatihOpen, setConfirmLatihOpen] = useState(false)

  async function jalankanLatihUlang() {
    if (!selectedRiwayat) return
    const target = selectedRiwayat
    setConfirmLatihOpen(false)
    onClose()
    ...
    <>
      <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button onClick={() => setConfirmLatihOpen(true)} className="mr-auto">
      ...
      {/* ── Konfirmasi Latih Ulang ── */}
      <AlertDialog open={confirmLatihOpen} onOpenChange={setConfirmLatihOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
              ...
              <div className="space-y-3 pt-1">
                ...
                {!selectedRiwayat.is_active && (
                  <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    <IconAlertTriangle size={14} className="text-amber-600 mt-0.5 shrink-0" />
                    <p className="text-xs text-amber-700 dark:text-amber-400">
                      Model ini sedang <strong>nonaktif</strong>. Melanjutkan akan mengaktifkan
                      model ini terlebih dahulu, menggantikan model aktif saat ini.
                    </p>
                  </div>
                )}

                <p className="text-xs">
                  Proses ini akan melatih ulang Decision Tree menggunakan data{' '}
                  <code className="text-[11px] bg-muted px-1 py-0.5 rounded">training_master</code>{' '}
                  terbaru, lalu <strong>mereklasifikasi seluruh hasil rekomendasi santri</strong>.
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={jalankanLatihUlang}>
              <IconBrain size={14} className="mr-1.5" />
              Ya, Latih Ulang
            </AlertDialogAction>
          </AlertDialogFooter>
