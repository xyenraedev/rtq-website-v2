...
export function TablePagination({ state }: { state: MonitoringSantriState }) {
  const { page, totalPages, goToPage, pageSize, changePageSize, pageSizeOptions, sorted } = state
  ...
  // Nomor halaman ditampilkan maksimal 5 tombol, dipusatkan di sekitar
  // halaman aktif supaya tidak melebar tak terkendali untuk data besar.
    ...
    <div className="px-4 py-3 border-t border-border bg-muted/20 flex flex-col sm:flex-row items-center justify-between gap-3">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <span>
          ...
          dari <span className="font-semibold text-foreground">{sorted.length}</span> santri
        </span>
        <select
          value={pageSize}
